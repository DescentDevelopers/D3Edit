...
void CRoomFrm::DeleteMarkedFaces() {
  ...
  if (num_m_faces == rp->num_faces && ROOMNUM(rp) < MAX_ROOMS) {
    OutrageMessageBox(
        "You cannot delete the only face in a level room. Unmark at least one face and try again.\n\n"
  ...
  /*
          else if (m_Current_nif_vert != -1)
          {
                  ...
                  if (which == PREV)
                          ...
                          (m_Current_nif_vert == last_non_face_vert) ? (m_Current_nif_vert = first_non_face_vert) :
     (next_non_face_vert); else Int3(); m_State_changed = true;
          }
  */
  else
...
bool CRoomFrm::GetModifiedStatus() { return m_Modified; }
...
void CRoomFrm::CloseRoom() { OnClose(); }
...
void CRoomFrm::PostNcDestroy() {
  ...
  m_State_changed =
      true; // force update of dialogs in OnIdle; should probably use a separate Editor_changed var, but oh well
...
void CRoomFrm::OnEditInsert() {
  ...
  case MODE_VERTEX:
    ...
    if (vertnum != -1) {
      PrintStatus("Vertex created at (%d, %d, %d).", (int)m_vec_InsertPos.x, (int)m_vec_InsertPos.y,
                  (int)m_vec_InsertPos.z);
      MarkVert(vertnum);
      m_Room_changed = true;
    } else
      ...
      if (obj_id != -1) {
        ...
        */
        ...
        if (ret == -1)
          ...
          OutrageMessageBox("Object inserted at %d, %d, %d", (int)m_vec_InsertPos.x, (int)m_vec_InsertPos.y,
                            (int)m_vec_InsertPos.z);
      }
    }
    break;
  ...
  case MODE_PATH:
    ...
    if (this == theApp.m_pRoomFrm) // only allow for the primary room frame
    {
      ...
      matrix orient = IDENTITY_MATRIX;
      if (Editor_state.GetCurrentNodeType() == NODETYPE_GAME)
        m_bHandle_Nodes ? ned_InsertNode(ROOMNUM(rp), m_vec_InsertPos, orient)
                        : ned_InsertPath(ROOMNUM(rp), m_vec_InsertPos, orient);
      else
...
void CRoomFrm::UnMarkAll() { OnUnmarkAll(); }

void CRoomFrm::CenterRoom() { OnCenterRoom(); }
...
char *CRoomFrm::GetPath() { return m_Path; }
...
void CRoomFrm::OnUpdateViewMoveCameraToCurrentObject(CCmdUI *pCmdUI) {
  ...
  if (theApp.m_pLevelWnd != NULL && this == theApp.m_pRoomFrm && Cur_object_index != -1 && Highest_object_index >= 0 &&
