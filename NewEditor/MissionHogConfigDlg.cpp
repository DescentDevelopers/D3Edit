...
CMissionHogConfigDlg::CMissionHogConfigDlg(CWnd *pParent) : CDialog(CMissionHogConfigDlg::IDD, pParent) {
...
void CMissionHogConfigDlg::OnOK() {
  ...
  if (MsnHogInfo.GetPathName(name)) {
    MessageBox("Since you have a mission hog selected, don't forget to set this hog in the correct spot of your "
               "msn/mn3 file so it is referenced correctly.  Also, make sure that you set this hog as your mission hog "
               "whenever you plan on working	on this level/mission.",
               name, MB_OK);
  }
