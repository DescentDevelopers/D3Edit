  ...
  if (rp != NULL) {
    ...
    if (num_m_faces > 0) {
      ...
      if (num_non_planars > 0) {
        int input =
            MessageBox("Some marked faces have been found to be non-planar. Would you like to triangulate these faces?",
                       "Triangulate non-planars?", MB_YESNO);
        if (input == IDYES) {
          int count = 0;
  ...
  if (rp != NULL && prim->face != -1 && prim->edge != -1) {
    ...
    if (num_m_verts == 1) {
      ...
      /*	TODO: add later
                              ...
                              {
                                      int input = MessageBox("Do you want to add the vertex to the edges of the adjacent
         faces as well?","Add to all adjacent faces?",MB_YESNO); if (input == IDYES)
                                      {
                                              int nextvert;
...
void CRoomDialogBar::OnFaceRemoveVert() {
  ...
  if (rp != NULL && prim->face != -1 && prim->vert != -1) {
    ...
    if (OutrageMessageBox(MBOX_YESNO, "Are you sure you want to remove vertex %d (%d) from face %d?", prim->vert,
                          rp->faces[prim->face].face_verts[prim->vert], prim->face) == IDYES) {
...
CMirrorDialog::CMirrorDialog(CWnd *pParent /*=NULL*/) : CDialog(CMirrorDialog::IDD, pParent) {
    ...
    if (num_m_verts == 2) {
      ...
      for (int i = 0; i < rp->faces[facenum].num_verts; i++) {
        if (rp->faces[facenum].face_verts[i] == marked_list[0])
          v0 = i;
        else if (rp->faces[facenum].face_verts[i] == marked_list[1])
          v1 = i;
      }
