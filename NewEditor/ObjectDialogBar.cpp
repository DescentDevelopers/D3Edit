...
CObjectBarList::CObjectBarList(CWnd *pParent) : CDialog(CObjectBarList::IDD, pParent) {
...
BOOL CObjectDialogBar::PreTranslateMessage(MSG *pMsg) { return CDialogBar::PreTranslateMessage(pMsg); }
...
int CObjectDialogBar::GetCurrentObject(void) { return m_CurrentObject; }
...
void LevelObjIncrementObject(int objnum) {
  ...
  if (Level_objects_in_use[objnum] == 1) {
    ...
    dlgObjectDialogBar->UpdateLevelObjectCount(Object_info[objnum].name, Level_objects_in_use[objnum],
                                               Level_objects_in_use[objnum] - 1);
  }
#endif
}
    ...
    if (wnd == theApp.m_pRoomFrm || (wnd == theApp.m_pLevelWnd && Insert_mode == 1)) {
      ...
      if (obj_id != -1) {
        ...
        */
        ...
        if (Insert_mode == 1)
          pos = theApp.m_pLevelWnd->m_Cam.target -
                (theApp.m_pLevelWnd->m_Cam.orient.fvec * theApp.m_pLevelWnd->m_Cam.dist);
        else
...
int CObjectDialogBar::ReadObjectList(CFILE *infile) {
  ...
  if (count != 0) {
    int answer =
        MessageBox("Choose Yes to replace the items in the list. Choose No to append.", "Replace Items?", MB_YESNO);
    if (answer == IDYES)
