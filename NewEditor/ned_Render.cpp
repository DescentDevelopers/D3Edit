...
// For textured view: determines if we should render all the rooms or only those within a certain portal distance from
// the current room
    ...
    if (!(fp->flags & FF_VISIBLE) || (fp->flags & FF_NOT_FACING)) {
      /*
         ...
         (GameTextures[fp->tmap].flags & TF_SPECULAR) &&
                                              ((fp->special_handle!=BAD_SPECIAL_FACE_INDEX) || (rp->flags &
         RF_EXTERNAL)))
                                              {
...
void RenderRoomObjects(room *rp) {
  ...
  for (objnum = rp->objects; (objnum != -1) && (n_objs < MAX_OBJECTS_PER_ROOM); objnum = Objects[objnum].next) {
    ...
    if (Render_mirror_for_room || (obj->type == OBJ_WEAPON && Weapons[obj->id].flags & WF_ELECTRICAL) ||
        IsPointVisible(&obj->pos, size, &zdist)) {
...
void DrawObjectSelectionBrackets(object *obj, bool front_flag) {
  ...
  for (int c = 0; c < 8; c++) {
    ...
    corner = (obj->orient.rvec * ((c & 1) ? mins.x : maxs.x)) + (obj->orient.uvec * ((c & 2) ? mins.y : maxs.y)) +
    ...
    if (obj->type == OBJ_POWERUP || obj->type == OBJ_ROBOT || obj->type == OBJ_CLUTTER) {
      ...
      */
      ...
      /*
                      ...
                      else if (obj->type==OBJ_PLAYER && !(Players[obj->id].flags &
         (PLAYER_FLAGS_DYING|PLAYER_FLAGS_DEAD)))
                      {
                              g3Point pnt;
  ...
  */
  ...
  /*
          else if (RenderObjectType==RO_GOURAUD || NoLightmaps)
          {
                  ...
                  if((obj->type == OBJ_ROBOT || obj->type==OBJ_PLAYER) && obj->rtype.pobj_info.subobj_flags !=
     0xFFFFFFFF) overlay = 1;

                  DrawPolygonModel (&obj_pos,&obj->orient,model_num,normalized_times,0, &RenderObject_LightDirection,
                                                                   RenderObjectStaticRedValue,
     RenderObjectStaticGreenValue,RenderObjectStaticBlueValue, obj->rtype.pobj_info.subobj_flags,use_effect,overlay);
          }
      ...
      /*
                                      else
                                      {
                                              ...
                                              float r = (float) GR_COLOR_RED(obj->rtype.sphere_color) / 255.0,
                                                              g = (float) GR_COLOR_GREEN(obj->rtype.sphere_color) /
         255.0, b = (float) GR_COLOR_BLUE(obj->rtype.sphere_color) / 255.0; DrawColoredDisk
         (&obj->pos,r,g,b,1,1,obj->size,0);
                                      }
      */
    }
#endif
  ...
  case RT_POLYOBJ:
    ...
    */
    ...
    /*
                            ...
                            if (obj->type==OBJ_PLAYER || obj->type==OBJ_ROBOT || (obj->type == OBJ_BUILDING &&
       obj->ai_info)) DrawSparkyDamageLightning (obj);
    */
    break;
  case RT_FIREBALL:
    /*
