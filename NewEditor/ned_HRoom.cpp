...
void ComputePlacedRoomMatrix() {
  ...
  mprintf((0, "orient: %f %f %f\n", vm_GetMagnitude(&Placed_room_orient.fvec),
           vm_GetMagnitude(&Placed_room_orient.rvec), vm_GetMagnitude(&Placed_room_orient.uvec)));
  ...
  mprintf((0, "rotmat: %f %f %f\n", vm_GetMagnitude(&Placed_room_rotmat.fvec),
           vm_GetMagnitude(&Placed_room_rotmat.rvec), vm_GetMagnitude(&Placed_room_rotmat.uvec)));
...
//					outbuf - the new polygon created by the part of the input polygon that was clipped
//away 					onv - the number of verys in outbuf 					num_vertices - pointer to the number of verts in the vertices array
void ClipAgainstEdge(int nv, short *vertnums, vertex *vertices, int *num_vertices, vector *v0, vector *v1,
                     vector *normal, short *inbuf, int *inv, short *outbuf, int *onv) {
