...
CTexAlignDialog::CTexAlignDialog(CWnd *pParent) : CDialog(CTexAlignDialog::IDD, pParent) {
...
CTexAlignDialog::CTexAlignDialog(prim *pprim, CWnd *pParent /*=NULL*/) : CDialog(CTexAlignDialog::IDD, pParent) {
  m_U = 0.0f;
  m_V = 0.0f;
  m_pPrim = pprim;
  //	m_bTimerSet = false;
...
void CTexAlignDialog::UpdateDialog() {
  ...
  if (m_pPrim == NULL ||
      m_pPrim->roomp == NULL) // || m_pPrim->face == -1 || m_pPrim->edge == -1 || m_pPrim->vert == -1)
  {
...
void PropagateFromMarkedFace(room *rp, short *facenums, int num_faces, int facenum, ubyte *face_flags,
                             bool matching_faces_only) {
  face *fp = &rp->faces[facenum];
