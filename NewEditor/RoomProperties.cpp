...
CRoomProperties::CRoomProperties(CWnd *pParent) : CDialog(CRoomProperties::IDD, pParent) {
...
void CRoomProperties::UpdateDialog() {
  ...
  cbox->SelectString(0, (LPCTSTR)((m_pPrim->roomp->ambient_sound == -1)
                                      ? NULL_NAME
                                      : AmbientSoundPatternName(m_pPrim->roomp->ambient_sound)));
...
BOOL CRoomProperties::OnInitDialog() {
  ...
  i = cbox->AddString("None");
  cbox->SetItemData(i, PD_NONE);
  i = cbox->AddString("Energy Weapon");
  cbox->SetItemData(i, PD_ENERGY_WEAPON);
  i = cbox->AddString("Matter Weapon");
  cbox->SetItemData(i, PD_MATTER_WEAPON);
  i = cbox->AddString("Melee Attack");
  cbox->SetItemData(i, PD_MELEE_ATTACK);
  i = cbox->AddString("Concussive Force");
  cbox->SetItemData(i, PD_CONCUSSIVE_FORCE);
  i = cbox->AddString("Wall Hit");
  cbox->SetItemData(i, PD_WALL_HIT);
  i = cbox->AddString("Volatile Hiss");
  cbox->SetItemData(i, PD_VOLATILE_HISS);
...
void CRoomProperties::OnFaceGoalCheck() {
  ...
  if (IsDlgButtonChecked(IDC_FACE_GOAL_CHECK)) {
    if (!(m_pPrim->roomp->flags & GOALROOM | RF_SPECIAL1 | RF_SPECIAL2 | RF_SPECIAL3 | RF_SPECIAL4 | RF_SPECIAL5 |
          RF_SPECIAL6)) {
...
void CRoomProperties::OnWindMake() {
  ...
  if (InputNumber(&num1, "Wind Start", "Enter first vertex number", this)) {
    ...
    if (InputNumber(&num2, "Wind Finish", "Enter second vertex number", this)) {
      ...
      OutrageMessageBox("Wind direction and speed will be calculated from the positions of the specified vertices. You "
                        "may change these values at any time.");
