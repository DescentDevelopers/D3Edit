...
void ned_DeleteNode() {
  ...
  if (GamePaths[path].num_nodes == 1) {
    if (MessageBox(NULL, "This is the only node in this path. Would you like to delete this path?.",
                   GamePaths[path].name, MB_YESNO) == IDYES)
      ned_DeletePath(true);
    return;
  }
...
void ned_InsertBNode(int roomnum, vector pos) {
  ...
  if (num_edges) {
    ...
    if (AfxMessageBox(
            "Would you like to create a node on the current edge? Answering NO will create a node with no edges.",
            MB_YESNO) == IDYES) {
