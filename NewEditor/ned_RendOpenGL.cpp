...
void rGL_SetWrapType(wrap_type val) { OpenGL_state.cur_wrap_type = val; }
...
void rGL_SetAlphaMultiplier() { Alpha_multiplier = rGL_GetAlphaMultiplier(); }
...
void rGL_SetFiltering(sbyte state) { OpenGL_state.cur_bilinear_state = state; }
...
//
// Clears the zbuffer
void rGL_ClearZBuffer() { dglClear(GL_DEPTH_BUFFER_BIT); }
...
void rGL_FillRect(ddgr_color color, int x1, int y1, int x2, int y2) {
  ...
  dglScissor(x1 + OpenGL_state.clip_x1, y1 + OpenGL_state.clip_y1, x2 + OpenGL_state.clip_x1,
             y2 + OpenGL_state.clip_y1);
  dglClearColor((float)r / 255.0f, (float)g / 255.0f, (float)b / 255.0f, 0);
  dglClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);
...
void rGL_SetFlatColor(ddgr_color color) { OpenGL_state.cur_color = color; }
  ...
  if (OpenGL_packed_pixels) {
    /*$
                    if (map_type==MAP_TYPE_LIGHTMAP)
                    {
                            ...
                            {
                                    dglTexSubImage2D
       (GL_TEXTURE_2D,0,0,0,size,size,GL_RGBA,UNSIGNED_SHORT_5_5_5_1_EXT,opengl_packed_Upload_data);
                            }
                            else
                                    dglTexImage2D
       (GL_TEXTURE_2D,0,GL_RGB5_A1,size,size,0,GL_RGBA,UNSIGNED_SHORT_5_5_5_1_EXT,opengl_packed_Upload_data);
                    }
                    else
    */
    {
    ...
    /*$
                    if (map_type==MAP_TYPE_LIGHTMAP)
                    {
                            ...
                            if (replace)
                                    dglTexSubImage2D
       (GL_TEXTURE_2D,0,0,0,size,size,GL_RGBA,GL_UNSIGNED_BYTE,opengl_Upload_data); else dglTexImage2D
       (GL_TEXTURE_2D,0,GL_RGBA,size,size,0,GL_RGBA,GL_UNSIGNED_BYTE,opengl_Upload_data);
                    }
                    else
    */
    {
...
// Sets the gamma correction value
void rGL_SetGammaValue(float val) { return; }
