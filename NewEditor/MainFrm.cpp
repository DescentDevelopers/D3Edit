...
CMainFrame::CMainFrame() { strD3EditDialogBarState = _T("D3EditDialogBarState"); }
...
void CMainFrame::SetStatusMessage(int index, char *str, bool update) { m_wndStatusBar.SetPaneText(index, str, update); }
...
void CMainFrame::OnUpdateStatusCurrent(CCmdUI *pCmdUI) {
  ...
  str.Format("R: %d/%d F: %d/%d P: %d/%d E: %d/%d V (idx): %d/%d (%d/%d)", roomnum, n_rooms, face, n_faces, portal,
             n_portals, edge, n_edges, vert, n_verts, vert_idx, n_room_verts);
  pCmdUI->SetText(str);
}
...
void CMainFrame::OnViewTextures() { ShowControlBar(&m_TextureDialog, !EkIsBarVisible(&m_TextureDialog), FALSE); }

void CMainFrame::OnUpdateViewTextures(CCmdUI *pCmdUI) { pCmdUI->SetCheck(EkIsBarVisible(&m_TextureDialog)); }
  ...
  if (theApp.m_pFocusedRoomFrm != NULL && theApp.m_pFocusedRoomFrm->m_Prim.roomp != NULL) {
    if (theApp.m_pFocusedRoomFrm->m_Prim.face != -1) {
      if (Curface != -1) {
        ...
        PlaceRoom(Curroomp, Curface, ROOMNUM(theApp.m_pFocusedRoomFrm->m_Prim.roomp),
                  theApp.m_pFocusedRoomFrm->m_Prim.face, -1);
      } else
...
void ned_VerifyMine() {
  ...
  // Go through the mine checking for errors, one error type at a time (we do this so that we can update the list box
  // regularly)
  ...
  if (errors)
    status->SetWindowText(
        "NOTE: Check the system's clipboard for detailed error info, as well as texture counts for this mine.");
  else
    status->SetWindowText(
        "NOTE: This mine has no recorded errors, but check the system's clipboard for texture counts.");
}
...
void ned_VerifyRoom(room *rp) {
  char str[256];
  int errors = 0, bad_normals, concave_faces, degenerate_faces, duplicate_faces, duplicate_points,
      duplicate_face_points, unused_points, nonplanar_faces, tjoints;
  ...
  // Interestingly, CListBox::AddString does not add the strings in the correct order, so we use CListBox::InsertString
  // instead.
  sprintf(str, "Found %d bad normals", bad_normals);
  lb->InsertString(-1, str);
  sprintf(str, "Found %d concave faces", concave_faces);
  ...
  if (errors)
    status->SetWindowText(
        "NOTE: Check the system's clipboard for detailed error info, as well as texture counts for this room.");
  else
    status->SetWindowText(
        "NOTE: This room has no recorded errors, but check the system's clipboard for texture counts.");
}
...
void CMainFrame::RoomProperties() { OnWindowRoomProperties(); }
...
void CMainFrame::OnTimer(UINT nIDEvent) {
  ...
  case TIMER_MINERVA:
    if (count == -1) {
      ...
      strcpy(Minerva.strings[2], "THIS IS JUST OUTRAGEOUS!! What were you THINKING?!! That someone wants to play your "
                                 "stupid Minerva clone?\n\n"
...
void CMainFrame::OnFileGravity() {
  ...
  sprintf(message,
          "Gravity is currently set to %.2f. Enter a new value for gravity (32.2 is Earth):", -Gravity_strength);
...
void CMainFrame::OnRoomPlaceTerrainRoom() {
  ...
  PlaceExternalRoom(cellnum, ROOMNUM(theApp.m_pFocusedRoomFrm->m_Prim.roomp), theApp.m_pFocusedRoomFrm->m_Prim.face,
                    (result == IDYES));
}
