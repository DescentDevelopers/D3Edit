...
CTexturePalette::CTexturePalette(CWnd *pParent /*=NULL*/) : CDialog(CTexturePalette::IDD, pParent) {
...
void CTexturePick::OnLButtonDown(UINT nFlags, CPoint point) {
  ...
  while ((m_ListItem > -1) && (num_items < (m_ListLen))) {
    ...
    if (point.x >= draw_x && point.x <= (draw_x + m_SurfDimension + 8) && point.y >= draw_y &&
        point.y <= (draw_y + m_SurfDimension + 8)) {
...
BOOL CTexturePalette::Create() { return CDialog::Create(m_nID, m_pParent); }
...
void CTexturePalette::PostNcDestroy() { delete this; }

BOOL CTexturePalette::DestroyWindow() { return CDialog::DestroyWindow(); }
...
void CTexturePick::OnItemSelected(int item) {
  ...
  if (item < 0 || item >= MAX_TEXTURES || !GameTextures[item].used ||
      !stricmp(GameTextures[item].name, "SAMPLE TEXTURE")) {
    wnd->SetWindowText("");
  } else {
...
void CTexturePick::SetShowFilter(int filter) { m_ShowFilter = filter; }
