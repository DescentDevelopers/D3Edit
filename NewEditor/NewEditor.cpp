...
BOOL CNewEditorApp::InitInstance() {
  ...
  if ((hog_lib_id = cf_OpenLibrary(hogpath)) != 0) {
    ...
    do {
      AfxMessageBox("Invalid Descent 3 directory!\nYou must set the Descent 3 directory in the settings dialog!",
                    MB_OK | MB_ICONSTOP);
      CSettingsDialog sd;
...
void CNewEditorApp::OnFileNew() {
  ...
  case 1:
    ...
    cFrame = (CLevelFrame *)pFrame->CreateNewChild(RUNTIME_CLASS(CLevelFrame), IDR_D3LTYPE, m_hMDI_D3L_Menu,
                                                   m_hMDI_D3L_Accel);
    ...
    rFrame =
        (CRoomFrm *)pFrame->CreateNewChild(RUNTIME_CLASS(CRoomFrm), IDR_ORFTYPE, m_hMDI_ORF_Menu, m_hMDI_ORF_Accel);
...
void CNewEditorApp::OpenNedFile(CString SelectedFile) {
  ...
  if (!strcmp(ext, ".d3l")) {
    ...
    CLevelFrame *cFrame = (CLevelFrame *)pFrame->CreateNewChild(RUNTIME_CLASS(CLevelFrame), IDR_D3LTYPE,
                                                                m_hMDI_D3L_Menu, m_hMDI_D3L_Accel);
    ...
    if (val != -1) {
      int answer =
          ::MessageBox(m_pMainWnd->m_hWnd, "Do you wish to overwrite the room in memory that already has this name?",
                       name, MB_YESNO);
      if (answer == IDYES) {
...
BOOL CNewEditorApp::OnIdle(LONG lCount) {
  ...
  if (m_pRoomFrm != NULL && (m_pRoomFrm->m_State_changed || m_pRoomFrm->m_Room_changed || World_changed)) {
    if (m_pRoomFrm->m_Room_changed) // TODO : must set m_Room_changed anytime modification to room is made..this is not
                                    // yet being done
    {
      ...
      sprintf(title, "%s - Current Room View - \"%s\"", Level_name,
              (Curroomp->name != NULL) ? Curroomp->name : "<none>");
  ...
  if (m_pFocusedRoomFrm != NULL && m_pFocusedRoomFrm != m_pRoomFrm &&
      (m_pFocusedRoomFrm->m_State_changed || m_pFocusedRoomFrm->m_Room_changed || World_changed)) {
    if (m_pFocusedRoomFrm->m_Room_changed) // TODO : must set m_Room_changed anytime modification to room is made..this
                                           // is not yet being done
    {
...
void CNewEditorApp::OnFileSave() {
  // This redirects to OnFileSaveRoom in case the accelerator (Ctrl+S) or toolbar button is pressed while a room window
  // is in focus
  void *wnd = AcquireWnd();
  if (wnd != NULL) {
    if (wnd == m_pLevelWnd) {
...
BOOL CAboutDlg::OnInitDialog() {
  ...
  m_SysInfo.Format("Memory:\r\nTotal Physical: %d\r\nFree Physical: %d\r\nFree Virtual: %d", ms.dwTotalPhys,
                   ms.dwAvailPhys, ms.dwAvailPageFile);
...
void CNewEditorApp::OnFileMruFile1() { OnFileMruFile(0); }

void CNewEditorApp::OnFileMruFile2() { OnFileMruFile(1); }

void CNewEditorApp::OnFileMruFile3() { OnFileMruFile(2); }

void CNewEditorApp::OnFileMruFile4() { OnFileMruFile(3); }
...
void CNewEditorApp::OnFileSaveasroom() {
  ...
  if ((strlen(name) + strlen(ext)) <= ROOM_NAME_LEN) {
    ...
    if (m_pFocusedRoomFrm != NULL) {
      if (m_pFocusedRoomFrm == m_pRoomFrm) {
        int input = AfxMessageBox(
            "The level's current room has been saved out as a separate room file (ORF). Would you like to open it now?",
            MB_YESNO);
        if (input == IDYES)
          OpenNedFile(SelectedFile);
      } else {
...
void CNewEditorApp::SetExtrudeDefaults(int which, float dist, BOOL delete_base_face, int inward, int faces,
                                       BOOL use_default) {
...
void CNewEditorApp::GetExtrudeDefaults(int *which, float *dist, BOOL *delete_base_face, int *inward, int *faces,
                                       BOOL *use_default) {
  ...
  if (version == 1) {
    while (!cfeof(infile)) {
      ...
      if (*filebuffer == '[') {
        ...
        if ((flags & CUSTOM_TEX_LIST) && strncmp(filebuffer, DER_TAG_TEXTURES, strlen(DER_TAG_TEXTURES)) == 0) {
          line_num += Editor_state.ReadTextureList(infile);
          continue;
        }

        if ((flags & CUSTOM_OBJ_LIST) && strncmp(filebuffer, DER_TAG_OBJECTS, strlen(DER_TAG_OBJECTS)) == 0) {
...
void DlgSaveResourceLists(DWORD flags) {
  ...
  // Need to do something with the default path
  _chdir(DefaultPath);
  fd = new CFileDialog(false, ".der", NULL, OFN_HIDEREADONLY | OFN_EXPLORER | OFN_ENABLEHOOK | OFN_ENABLETEMPLATE,
                       szFilter);
...
void DlgOpenResourceLists(DWORD flags) {
  ...
  // Need to do something with the default path
  _chdir(DefaultPath);
  fd = new CFileDialog(true, ".der", NULL,
                       OFN_FILEMUSTEXIST | OFN_HIDEREADONLY | OFN_EXPLORER | OFN_ENABLEHOOK | OFN_ENABLETEMPLATE,
                       szFilter);
