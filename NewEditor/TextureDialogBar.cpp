...
CTextureBarList::CTextureBarList(CWnd *pParent) : CDialog(CTextureBarList::IDD, pParent) {
...
BOOL CTextureDialogBar::PreTranslateMessage(MSG *pMsg) { return CDialogBar::PreTranslateMessage(pMsg); }
...
void CTextureDialogBar::PaintPreviewArea(CWnd *pWnd) {
  ...
  if (pWnd == &m_CTLightView) {
    ...
    if (IsTextureValid(m_CurrentTexture)) {
      ...
      // Scaled color
      intensity = max(max(GameTextures[m_CurrentTexture].r, GameTextures[m_CurrentTexture].g),
                      GameTextures[m_CurrentTexture].b);
      if (intensity >= .001) {
        r /= intensity;
        g /= intensity;
        b /= intensity;
...
void LevelTexIncrementTexture(int texnum) {
  ...
  if (Level_textures_in_use[texnum] == 1) {
    ...
    dlgTextureDialogBar->UpdateLevelTextureCount(GameTextures[texnum].name, Level_textures_in_use[texnum],
                                                 Level_textures_in_use[texnum] - 1);
  }
#endif
}
