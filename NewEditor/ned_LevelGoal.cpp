...
void lgoal::GoalComplete(int handle, bool announce) {
  ...
  //
  AddPersistentHUDMessage(GOAL_MESSAGE_COLOR,HUD_MSG_PERSISTENT_CENTER,y,GOAL_MESSAGE_TIME,m_completion_message); char
  message[200]; sprintf(message,"%s %s",TXT_COMPLETED_HUD,m_name); if(Demo_flags==DF_RECORDING)
                  {
                          ...
                          if(flags & LGF_ENABLED)
                          {
                                  ...
                                  if(ctype == LIT_TERRAIN_CELL)
                                  {
                                          if(Game_mode & GM_MULTI)
                                          {
                                                  ...
                                                  {
                                                          if((NetPlayers[j].flags & NPF_CONNECTED) &&
     (NetPlayers[j].sequence >= NETSEQ_PLAYING))
                                                          {
