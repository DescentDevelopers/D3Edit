...
// Movement controls
#define PANNING                                                                                                        \
  ((m_Mouse.left && m_Keys.shift && !m_Keys.ctrl) || (m_Mouse.mid && !m_Keys.ctrl && !m_Keys.shift && !m_Keys.alt))
#define ZOOMING                                                                                                        \
  ((m_Mouse.left && m_Keys.shift && m_Keys.ctrl) || (m_Mouse.mid && !m_Keys.ctrl && m_Keys.shift && !m_Keys.alt))
#define ROTATING                                                                                                       \
  ((m_Mouse.left && !m_Keys.shift && m_Keys.ctrl) || (m_Mouse.mid && !m_Keys.shift && m_Keys.ctrl && !m_Keys.alt))
#define RADDING                                                                                                        \
  ((m_Mouse.left && m_Keys.alt && m_Keys.shift) || (m_Mouse.mid && m_Keys.alt && !m_Keys.ctrl && !m_Keys.shift))
...
void Cned_LevelWnd::Render(bool force_repaint) {
  ...
  if (m_bTextured) {
    ...
    //		TGWRenderMine(&Viewer_object->pos,&Viewer_object->orient,D3_DEFAULT_ZOOM,ROOMNUM(m_Prim.roomp)); //
    //D3EditState.current_room);
    if (!Dont_draw_terrain)
      RenderTerrain(0);
    DrawAllPaths(m_grViewport, &Viewer_object->pos, &Viewer_object->orient, D3_DEFAULT_ZOOM);
    EBNode_Draw(EBDRAW_LEVEL, m_grViewport, &Viewer_object->pos, &Viewer_object->orient,
                D3_DEFAULT_ZOOM); // EBN_draw_type
    TexGrStopOpenGL();
  } else {
...
void Cned_LevelWnd::OnLButtonDown(UINT nFlags, CPoint point) {
  ...
  if (!m_bTextured) {
    ...
    if (Placed_room == -1) {
      // WireframeFindRoomFace requires that the viewport its checking to be the last one that was rendered, so call
      // Render() first.
      Render(false);
  ...
  } else // Textures
  {
    ...
    if (rc.PtInRect(point)) {
      ...
      TGWRenderMine(&view_pos, &m_Cam.orient, D3_DEFAULT_ZOOM, ROOMNUM(m_Prim.roomp)); // D3EditState.current_room);
                                                                                       //		TGWRenderMine
      //(&Viewer_object->pos,&Viewer_object->orient,D3_DEFAULT_ZOOM,Viewer_object->roomnum);
      if (!Dont_draw_terrain)
        RenderTerrain(0);
      DrawAllPaths(m_grViewport, &Viewer_object->pos, &Viewer_object->orient, D3_DEFAULT_ZOOM);
      EBNode_Draw(EBDRAW_LEVEL, m_grViewport, &Viewer_object->pos, &Viewer_object->orient,
      ...
      if (TSearch_found_type == TSEARCH_FOUND_OBJECT) {
        ...
        //			ObjMoveManager.Start(this, m_grViewport->width(), m_grViewport->height(), &m_ViewPos,
        //&m_ViewMatrix, point.x, point.y);
      } else if (TSearch_found_type == TSEARCH_FOUND_NODE) {
        ...
        State_changed = true;
      } else if (TSearch_found_type == TSEARCH_FOUND_BNODE) {
        // Found a Bnode!
        ...
        State_changed = true;
      } else if (TSearch_found_type == TSEARCH_FOUND_SKY_DOME) {
        if (nFlags & MK_SHIFT) {
      ...
      } else if (TSearch_found_type == TSEARCH_FOUND_SATELLITE) {
        if (nFlags & MK_SHIFT) {
...
void Cned_LevelWnd::OnLButtonUp(UINT nFlags, CPoint point) {
  ...
  if (!m_bTextured) {
    ...
    if (rc.PtInRect(point)) {
            ...
            if ((roomnum == ROOMNUM(Markedroomp)) && (facenum == Markedface))
              ...
              if (HTextureCopyUVsToFace(&Rooms[roomnum], facenum, Markedroomp, Markedface, count)) {
                ...
                PrintStatus("Texture & UVs copied from room %d face %d to room %d face %d (rotation = %d/%d)\n",
                            ROOMNUM(Markedroomp), Markedface, roomnum, facenum,
                            count % Markedroomp->faces[Markedface].num_verts, Markedroomp->faces[Markedface].num_verts);
                World_changed = 1;
              } else
      ...
      } else if (TSearch_found_type == TSEARCH_FOUND_TERRAIN) {
        if (nFlags & MK_SHIFT) {
...
void Cned_LevelWnd::OnKeyDown(UINT nChar, UINT nRepCnt, UINT nFlags) {
  ...
  case 0x4D: // 'M'
    if (HIWORD(::GetKeyState(VK_SHIFT)))
      ...
      if (rp == Markedroomp) {
        ...
        PrintStatus("Room:face:edge:vertex %d:%d:%d:%d marked.", ROOMNUM(Markedroomp), Markedface, Markededge,
                    Markedvert);
        State_changed = 1;
      }
    }
  ...
  case 0x56: // 'V'
    ...
    x = rp->verts[vert_idx].x;
    y = rp->verts[vert_idx].y;
    z = rp->verts[vert_idx].z;
...
void Cned_LevelWnd::OnDisplayCurrentRoomView() {
  ...
  if (theApp.m_pRoomFrm == NULL) {
    ...
    CRoomFrm *rFrame = (CRoomFrm *)pFrame->CreateNewChild(RUNTIME_CLASS(CRoomFrm), IDR_ORFTYPE, theApp.m_hMDI_ORF_Menu,
                                                          theApp.m_hMDI_ORF_Accel);
...
bool Cned_LevelWnd::GetModifiedStatus() { return m_Modified; }
