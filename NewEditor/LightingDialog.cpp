...
CLightingDialog::CLightingDialog(CWnd *pParent) : CDialog(CLightingDialog::IDD, pParent) {
...
void CLightingDialog::OnLightsInMine() {
  ...
  sprintf(str,
          "There are %d lights present in the mine.  Do you wish for me to set the 'Iterations' control to match this "
          "number?",
          lightcount);
...
void CLightingDialog::OnMineRadiosity() {
  ...
  MessageBox("I will now calculate radiosity for the indoor engine.  This might take a few minutes to setup, but "
             "afterwards you will be able to cancel if you wish.",
             "Message", MB_OK);
...
void CLightingDialog::OnResetMultiples() {
  // TODO: Add your control notification handler code here
  int answer =
      MessageBox("Are you sure you wish to reset all light multiples in the mine?", "Light Question", MB_YESNO);
  if (answer == IDNO)
    return;
...
void CLightingDialog::OnRoomResetMultiples() {
  ...
  int i;
  int answer =
      MessageBox("Are you sure you wish to reset all light multiples in the current room?", "Light Question", MB_YESNO);
  if (answer == IDNO)
    return;
...
void CLightingDialog::OnRoomCountLights() {
  ...
  sprintf(str,
          "There are %d lights present in the current room.  Do you wish for me to set the 'Iterations' control to "
          "match this number?",
          lightcount);
  int answer = MessageBox(str, "Light Question", MB_YESNO);
  if (answer == IDNO)
