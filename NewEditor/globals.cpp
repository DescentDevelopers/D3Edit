...
char *GetErrorMessage() { return Editor_error_message; }
...
int GetTextureBitmap(int handle, int framenum, bool force) { return ned_GetTextureBitmap(handle, framenum, force); }

int FindSoundName(char *p) { return ned_FindSound(p); }

int FindTextureName(char *p) { return ned_FindTexture(p); }

int FindObjectIDName(char *p) { return ned_FindObjectID(p); }

int FindDoorName(char *p) { return ned_FindDoor(p); }
...
grSurface *CEditorState::GetDesktopSurface(void) { return Desktop_surf; }
...
bool CEditorState::RegisterTextureDropWindow(CWnd *wnd) { return (bool)(m_TextureDrop.Register(wnd) != 0); }
...
int CEditorState::GetCurrentNodeType() { return nodetype; }

// sets the current node type
void CEditorState::SetCurrentNodeType(int type) { nodetype = type; }

// gets the current path
int CEditorState::GetCurrentPath() { return path; }

// gets the current node
int CEditorState::GetCurrentNode() { return node; }

// sets the current path
void CEditorState::SetCurrentPath(int p) { path = p; }

// sets the current node
void CEditorState::SetCurrentNode(int n) { node = n; }
...
DROPEFFECT CTextureOLETarget::OnDropEx(CWnd *pWnd, COleDataObject *pDataObject, DROPEFFECT dropDefault,
...
bool CEditorState::RegisterGenericDropWindow(CWnd *wnd) { return (bool)(m_GenericDrop.Register(wnd) != 0); }
...
DROPEFFECT CGenericOLETarget::OnDropEx(CWnd *pWnd, COleDataObject *pDataObject, DROPEFFECT dropDefault,
                                       DROPEFFECT dropList, CPoint point) {
  UINT format = RegisterClipboardFormat("GenericDrop");
  if (pDataObject->IsDataAvailable(format)) {
    HGLOBAL hmem = pDataObject->GetGlobalData(format);
...
bool CreateStringTable(char *file, char ***ptr, int *num) { return false; }

void Localization_SetLanguage(int i) {}

int Localization_GetLanguage(void) { return 1; }
