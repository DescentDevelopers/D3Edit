...
bool RotateVerts(room *rp, short *list, int num_verts, angle ang_x, angle ang_y, angle ang_z, vector offset) {
  ...
  //			mprintf(0,"Room %3d face %3d is concave at vertex %d",ROOMNUM(rp),i,vert);
          ...
          if (bConcave)
          {
                  int answer = OutrageMessageBox(MB_YESNOCANCEL,"One or more faces would become concave as a result of
  this move.\nDo you want to split faces to resolve concavities?"); if (answer == IDYES)
  ...
  if (rp->num_verts > (MAX_VERTS_PER_ROOM - 1)) {
    ...
    if (rp->num_faces > (MAX_FACES_PER_ROOM - 1)) {
      ...
      for (i = 0; i < num_new_verts; i++) {
        ...
        /*
                                        if ( !ok || (fp->face_verts[3] - fp->face_verts[0]) || (fp->face_verts[2] -
           fp->face_verts[1]) );
                                        {
                                                // Had an error; we've got to compute the normal ourselves (this is a
           hack for faces extruded a distance of 0) fp->normal =
                                        }
        */
        // Assign UVs
...
//									relative to their own faces.  The shared edge is verts
//<vn0,vn0+1> on 									face 0, and <vn1+1,vn1> on face 1 Returns:		true if a shared edge was found, else false
