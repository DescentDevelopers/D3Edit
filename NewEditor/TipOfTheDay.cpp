...
CTipOfTheDay::CTipOfTheDay(CWnd *pParent) : CDialog(CTipOfTheDay::IDD, pParent) {
  ...
  if (cfp) {
    do {
      ...
      if ((szline[0] == '&') && (szline[1] == '&')) {
        ...
        m_lpszTips[i] =
            (char *)mem_realloc(m_lpszTips[i], strlen(m_lpszTips[i]) + strlen(szline) + strlen(TIP_NEWLINE) + 1);
        if (*m_lpszTips[i])
