...
void Cned_PerspWnd::Render(bool force_repaint) {
  ...
  if (m_bTextured) {
    ...
    //		TGWRenderMine(&Viewer_object->pos,&Viewer_object->orient,D3_DEFAULT_ZOOM,ROOMNUM(m_pPrim->roomp)); //
...
void Cned_PerspWnd::OnUpdateViewMoveCameraToCurrentObject(CCmdUI *pCmdUI) {
  ...
  if (theApp.m_pLevelWnd != NULL && m_pParentFrame == theApp.m_pRoomFrm && Cur_object_index != -1 &&
      Highest_object_index >= 0 && Objects[Cur_object_index].roomnum == ROOMNUM(m_pPrim->roomp))
    enable = true;
