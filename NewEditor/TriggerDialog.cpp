...
CTriggerDialog::CTriggerDialog(CWnd *pParent) : CDialog(CTriggerDialog::IDD, pParent) {
...
void CTriggerDialog::OnTrigAddTrigger() {
  ...
  if ((rp->num_portals > 0) && !(rp->flags & RF_EXTERNAL)) {
    ...
    if (shell_errors)
      OutrageMessageBox("The current room has a bad shell. You will not be able to make this trigger a floating "
                        "trigger until the shell is repaired.");
  }
