...
public:
  CHogfilelistmgr() { Hogfile_list = NULL; }
  ~CHogfilelistmgr() {
  ...
  void GetFileList(void (*callback)(char *filename, void *data), void *data, char *wildcard,
                   char *hog_file /*NULL if all*/) {
    tHogfilenode *curr;
    curr = Hogfile_list;
...
CHogBrowser::CHogBrowser(char *wildcard_list /*=NULL*/, CWnd *pParent /*=NULL*/) : CDialog(CHogBrowser::IDD, pParent) {
...
void CHogBrowser::OnShowWindow(BOOL bShow, UINT nStatus) { CDialog::OnShowWindow(bShow, nStatus); }
...
void CHogBrowser::OnSelchangeFiletype() { OnSelchangeHogfileChoose(); }
...
void RegisterHogFile(char *filename, int lib_id) { Hog_mgr.AddHog(lib_id, filename); }

void UnRegisterHogFile(int lib_id) { Hog_mgr.DelHog(lib_id); }
