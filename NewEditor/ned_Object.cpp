...
int ObjInitGeneric(object *objp, bool reinit) {
  ...
  */
  ...
  /*
          ...
          //Set anim frame
          objp->rtype.pobj_info.anim_frame = ((1.0f - r_val) *
     (float)Object_info[objp->id].anim[MC_STANDING][AS_ALERT].from) + (r_val *
     (float)Object_info[objp->id].anim[MC_STANDING][AS_ALERT].to);
  */
...
int ned_AllocObjectInfo(char *name, int tablefile) {
  ...
  for (i = 0; i < MAX_OBJECT_IDS; i++) {
    if (!Object_info[i].used) {
      ...
      for (int j = 0; j < MAX_LOADED_TABLE_FILES; j++)
...
void ObjSetPos(object *obj, vector *pos, int roomnum, matrix *orient, bool f_update_attached_children) {
  ...
  if (obj->roomnum != roomnum) {
    ...
    /*
                    ...
                    if(obj->type == OBJ_PLAYER && !ROOMNUM_OUTSIDE(roomnum) && (Rooms[roomnum].flags &
       RF_INFORM_RELINK_TO_LG))
                    {
