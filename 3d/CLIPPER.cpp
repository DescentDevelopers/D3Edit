...
int free_point_num=-1;

g3Point temp_points[MAX_POINTS_IN_POLY];
g3Point *free_points[MAX_POINTS_IN_POLY];
...
g3Point *ClipCustomEdge (g3Point *on_pnt,g3Point *off_pnt)
{
	...
	ray_direction=(off_pnt->p3_vec-on_pnt->p3_vec);
	ray_direction.x/=Matrix_scale.x;
	ray_direction.y/=Matrix_scale.y;
	ray_direction.z/=Matrix_scale.z;
		
	w=on_pnt->p3_vec-Clip_plane_point;
	w.x/=Matrix_scale.x;
	w.y/=Matrix_scale.y;
	w.z/=Matrix_scale.z;
	
	den = -(Clip_plane * ray_direction);
	if ( den == 0.0f )
		k = 1.0f;
	else 
	{
...
int ClipPlane(int plane_flag,g3Point **src,g3Point **dest,int *nv,g3Codes *cc)
{
	...
	for (i=0,prev=*nv-1,next=1;i<*nv;i++) {
		...
		if (src[i]->p3_codes & plane_flag) {				//cur point off?
			...
			if (! (src[prev]->p3_codes & plane_flag)) {	//prev not off?
