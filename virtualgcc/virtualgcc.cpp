...
void main(int argc, char *argv[]) {
  ...
  if (FindGCCPath(gcc_bin_path)) {
    ...
    output("gcc.exe not found in your path, please verify that your PATH environment variable is setup correctly, and "
           "it includes your bin directory of your gcc installation\n");
    output("example:\n");
...
bool Step1(STARTUPINFO *si) {
  ...
  // build script
  sprintf(command_line, "gcc.exe -I. -I%s -mwindows -o %s -c %s %s %s %s", Working_directory, output_object,
          Source_filename, WarningLevelString, ChecksumString, DebugLevelString);
  ...
  sprintf(command_line, "gcc.exe -I. -I%s -mwindows -o %s -c %s %s %s", Working_directory, output_dllinit,
          DLLInit_filename, WarningLevelString, DebugLevelString);
  bRet = CreateProcess(NULL, command_line, NULL, NULL, TRUE, 0, NULL, NULL, si, &pi);
  if (bRet) {
    WaitForSingleObject(pi.hProcess, INFINITE);
    CloseHandle(pi.hProcess);
...
void Step2(STARTUPINFO *si) {
  ...
  // build up the command line
  char command_line[256];
  sprintf(command_line, "dllwrap.exe --export-all --def %s -o %s %s %s -mwindows", Def_filename, Output_filename,
          output_object, output_dllinit);
