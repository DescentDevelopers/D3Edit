...
// --------------------------
const GUID *effectGUID[kMaxEffectSubTypes] = {&GUID_ConstantForce, &GUID_RampForce, &GUID_CustomForce,
                                              // period
                                              &GUID_Square, &GUID_Sine, &GUID_Triangle, &GUID_SawtoothUp,
                                              &GUID_SawtoothDown,
                                              // condition
                                              &GUID_Spring, &GUID_Damper, &GUID_Inertia, &GUID_Friction};
...
void *getwindowhandle(void) { return DInputData.hwnd; }
...
LPDIRECTINPUTDEVICE2 ddio_ff_get_joystick_obj(tDevice dev) { return DID2_Joy[(int)dev]; }
...
int ddio_ffjoy_Query(int dev, int *but_flags, int *axis_flags) {
  ...
  DWORD DIAxisOFS[6] = {DIJOFS_X, DIJOFS_Y, DIJOFS_Z, DIJOFS_RX, DIJOFS_SLIDER(0), DIJOFS_SLIDER(1)};
...
// -------------------------------------------------------------------
void ddio_ffb_DestroyAll(void) { numEffects = 0; }
...
int ddio_ffb_effectCreate(tDevice dev, tFFB_Effect *eff) {
  ...
  ddEffect[numEffects].general.dwTriggerButton =
  ...
  hr = IDirectInputDevice2_CreateEffect(DID2_Joy[dev], *effectGUID[eff->Type], (LPCDIEFFECT)&ddEffect[numEffects],
...
static int ddio_ffjoy_AcquireErr(HRESULT res, int dev_num) {
  if (res == DI_OK) {
    ...
    return TRUE;
  } else if (res == DIERR_INVALIDPARAM) {
    mprintf((0, "device %d DIERR_INVALIDPARAM\n", dev_num));
    return FALSE;
  } else if (res == DIERR_OTHERAPPHASPRIO) {
...
typedef LPDIRECTINPUTEFFECT *(__stdcall *IFCreateEffects_fp)(HIFORCEPROJECT hProject, LPCSTR pObjectName,
                                                             int *pNumEffects);
...
void ddio_LoadImmersionDLL(void) {
  ...
  if (!d_IFLoadProjectFile || !d_IFReleaseProject || !d_IFCreateEffects) {
...
void PrintDirectInputErrorString(HRESULT hr, char *format, ...) {
  ...
  if (hr == DI_BUFFEROVERFLOW)
    strcat(buffer, "*The device buffer overflowed and some input was lost. This value is equal to the S_FALSE standard "
                   "COM return value.\n");
  if (hr == DI_DOWNLOADSKIPPED)
    strcat(buffer, "*The parameters of the effect were successfully updated, but the effect could not be downloaded "
                   "because the associated device was not acquired in exclusive mode. \n");
  ...
  if (hr == DI_NOTATTACHED)
    strcat(buffer, "*The device exists but is not currently attached. This value is equal to the S_FALSE standard COM "
                   "return value.\n");
  if (hr == DI_OK)
    strcat(buffer,
           "*The operation completed successfully. This value is equal to the S_OK standard COM return value.\n");
  if (hr == DI_POLLEDDEVICE)
    strcat(buffer, "*The device is a polled device. As a result, device buffering will not collect any data and event "
                   "notifications will not be signaled until the IDirectInputDevice2::Poll method is called.\n");
  if (hr == DI_PROPNOEFFECT)
    strcat(buffer, "*The change in device properties had no effect. This value is equal to the S_FALSE standard COM "
                   "return value.\n");
  if (hr == DI_TRUNCATED)
    strcat(buffer, "*The parameters of the effect were successfully updated, but some of them were beyond the "
                   "capabilities of the device and were truncated to the nearest supported value.\n");
  ...
  if (hr == DIERR_BADDRIVERVER)
    strcat(buffer, "*The object could not be created due to an incompatible driver version or mismatched or incomplete "
                   "driver components.\n");
  ...
  if (hr == DIERR_DEVICENOTREG)
    strcat(buffer, "*The device or device instance is not registered with DirectInput. This value is equal to the "
                   "REGDB_E_CLASSNOTREG standard COM return value.\n");
  if (hr == DIERR_EFFECTPLAYING)
    strcat(buffer, "*The parameters were updated in memory but were not downloaded to the device because the device "
                   "does not support updating an effect while it is still playing. \n");
  if (hr == DIERR_HASEFFECTS)
    strcat(buffer, "*The device cannot be reinitialized because there are still effects attached to it.\n");
  if (hr == DIERR_GENERIC)
    strcat(buffer, "*An undetermined error occurred inside the DirectInput subsystem. This value is equal to the "
                   "E_FAIL standard COM return value.\n");
  if (hr == DIERR_HANDLEEXISTS)
    strcat(buffer, "*The device already has an event notification associated with it. This value is equal to the "
                   "E_ACCESSDENIED standard COM return value.\n");
  if (hr == DIERR_INCOMPLETEEFFECT)
    strcat(buffer, "*The effect could not be downloaded because essential information is missing. For example, no axes "
                   "have been associated with the effect, or no type-specific information has been supplied.\n");
  if (hr == DIERR_INPUTLOST)
    strcat(buffer, "*Access to the input device has been lost. It must be reacquired.\n");
  if (hr == DIERR_INVALIDPARAM)
    strcat(buffer,
           "*An invalid parameter was passed to the returning function, or the object was not in a state that "
           "permitted the function to be called. This value is equal to the E_INVALIDARG standard COM return value.\n");
  ...
  if (hr == DIERR_NOINTERFACE)
    strcat(buffer, "*The specified interface is not supported by the object. This value is equal to the E_NOINTERFACE "
  ...
  if (hr == DIERR_OTHERAPPHASPRIO)
    strcat(buffer,
           "*Another application has a higher priority level, preventing this call from succeeding. This value is "
           "equal to the E_ACCESSDENIED standard COM return value. This error can be returned when an application has "
           "only foreground access to a device but is attempting to acquire the device while in the background. \n");
  if (hr == DIERR_OUTOFMEMORY)
    strcat(buffer, "*The DirectInput subsystem couldn't allocate sufficient memory to complete the call. This value is "
                   "equal to the E_OUTOFMEMORY standard COM return value.\n");
  if (hr == DIERR_READONLY)
    strcat(buffer, "*The specified property cannot be changed. This value is equal to the E_ACCESSDENIED standard COM "
                   "return value.\n");
  ...
  if (hr == DIERR_UNSUPPORTED)
    strcat(buffer, "*The function called is not supported at this time. This value is equal to the E_NOTIMPL standard "
                   "COM return value. \n");
  if (hr == E_PENDING)
