...
void editorSelectorManager::StartSelection(CWnd *wnd, void (*func)(editorSelectorManager *), int x, int y) {
  ...
  if (m_DragState != 1) {
    ...
    if (m_DragState == 2) {
      ...
      if (r_b < r_t)
        SWAP(r_t, r_b);
      if (r_l < client_rect.left)
        r_l = client_rect.left;
      if (r_t < client_rect.top)
        r_t = client_rect.top;
      if (r_r > client_rect.right)
        r_r = client_rect.right;
      if (r_b > client_rect.bottom)
        r_b = client_rect.bottom;
...
void editorSelectorManager::GetSelectedRect(int *l, int *t, int *r, int *b) {
  ...
  if (m_b < m_t)
    SWAP(m_t, m_b);
  if (m_l < client_rect.left)
    m_l = client_rect.left;
  if (m_t < client_rect.top)
    m_t = client_rect.top;
  if (m_r > client_rect.right)
    m_r = client_rect.right;
  if (m_b > client_rect.bottom)
    m_b = client_rect.bottom;
