...
void CDallasMainDlg::DisplayActionHeaderNodeMenu(POINT *point, int level_type, int clause_type, bool can_add_else) {
  ...
  if (level_type == TOP_LEVEL)
    ...
    main_menu.AppendMenu(MF_GRAYED | MF_POPUP, (unsigned int)select_extimes_submenu.Detach(),
  ...
  if (level_type == TOP_LEVEL)
    ...
    main_menu.AppendMenu(MF_GRAYED | MF_POPUP, (unsigned int)select_chain_submenu.Detach(),
