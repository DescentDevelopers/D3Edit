...
void (*Texture_functions[])(g3Point *, int) = {
    tex_UnlitTexturedQuad,   tex_LitTexturedQuad,        tex_ColoredTexturedQuad, tex_PerUnlitTexturedQuad,
    tex_PerLitTexturedQuad,  tex_UnlitTransQuad,         tex_LitTransQuad,        tex_PerUnlitTransQuad,
    tex_PerLitTransQuad,     tex_PerColoredTexturedQuad, tex_LinFlatShade,        tex_PerFlatShade,
...
void tex_SetMipState(int mipstate) { Mips_on = mipstate; }
...
void tex_SetFlatColor(ddgr_color color) { Flat_polygon_color = GR_COLOR_TO_16(color); }
...
void tex_ColoredTexturedQuad(g3Point *p, int nv) { Int3(); }
...
void tex_PerColoredTexturedQuad(g3Point *p, int nv) { Int3(); }
...
void tex_SetFogState(int on) { Fog_state = on; }
...
void tex_SetLighting(light_state state) { tex_LightingState = state; }
void tex_SetColorModel(color_model state) { tex_ColorModel = state; }
void tex_SetTextureType(texture_type state) { tex_TextureType = state; }
...
void tex_SetAlphaType(sbyte atype) { tex_alpha_type = atype; }
...
void tex_ReleaseLFBLock(renderer_lfb *lfb) { lfb->data = NULL; }
...
inline void tex_SetPixelClip16(ushort col, int x, int y) {
  if (x >= Tex_x2 || x < Tex_x1 || y >= Tex_y2 || y < Tex_y1)
    return;
  ushort *ptr = (ushort *)Tex_framebuffer + (y * (Tex_pitch >> 1)) + x;
  *ptr = col;
}
...
int tex_ClipRect(int &l, int &t, int &r, int &b) {
  ...
  if (l >= Tex_x2 || t >= Tex_y2 || r < Tex_x1 || b < Tex_y1)
    return 2;
