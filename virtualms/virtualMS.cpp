...
int Step1(STARTUPINFO *si) {
  ...
  sprintf(command_line, "cl.exe %s /nologo /MTd %s %s %s /Fo\"%s\" /GD /Gd /c", Source_filename, WarningLevelString,
          DebugLevelString, ChecksumString, output_object);
...
void Step2(STARTUPINFO *si) {
  ...
  sprintf(
      command_line,
      "link.exe \"%s\" /nologo /dll %s /subsystem:windows /machine:I386 /def:\"%s\" /out:%s /INCREMENTAL:no /PDB:none",
      output_object, DebugLevelString, Def_filename, Output_filename);
