...
bool osWinObject::create(osObject *parent_os, void *info) {
  ...
  hwnd = CreateWindowEx(dw, (LPCSTR)winfo->name, (LPCSTR)winfo->name, WS_POPUP | WS_SYSMENU, 0, 0, m_WinWidth,
                        m_WinHeight, NULL, NULL, hinst, NULL);
...
void osWinObject::get_info(void *info, int size_str) { memcpy(info, &m_WinInfo, size_str); }
...
void osWinObject::os_init() {
  ...
  // Detect if mmx is present
        __asm
	{
        ...
        if (mmx_present) mprintf((0, "MMX technology found!\n"));
        else mprintf((0, "MMX not detected.\n"));
