            ...
            if (p->type != OBJ_GHOST) {
              ...
              if (croom == p->roomnum) {
                ...
                if (dist < MATCEN_DAMAGE_DIST) {
                  ...
                  if (p == Player_object)
                    PhysicsDoSimLinear(p, &pos, &dir, &p->mtype.phys_info.velocity, &movement_vec, &movement_pos,
                                       Frametime, 1);
                }
              }
            }
        ...
        if (p->type != OBJ_GHOST) {
          ...
          if (croom == p->roomnum) {
            ...
            if (dist < MATCEN_DAMAGE_DIST) {
              ...
              ApplyDamageToPlayer(p, p, PD_ENERGY_WEAPON, damage);
              PhysicsDoSimLinear(p, &pos, &dir, &p->mtype.phys_info.velocity, &movement_vec, &movement_pos, Frametime,
                                 1);
            }
          }
        }
